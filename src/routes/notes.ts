...
notesRoute.patch('/:id', async (c) => {
  ...
  const updated = await collections.notes(db).findOne({ _id: id });
...
notesRoute.delete('/:id', async (c) => {
  ...
  await collections.notes(db).updateOne({ _id: id }, { $set: { deleted: true, updatedAt: new Date() } });
...
// ---------------------------------------------------------------------------
const remoteNoteRowSchema = z.object({
  id: z.string().uuid(),
...
type RemoteNoteRow = z.infer<typeof remoteNoteRowSchema>;
...
notesRoute.post('/push', async (c) => {
  ...
  const newActiveCount = rows.filter((r) => !existingById.has(r.id) && !r.deleted).length;
  ...
  for (const row of rows) {
    try {
      ...
      if (row.deleted) await deleteNoteFile(accessToken, refreshToken, driveFileId).catch(() => {});
      ...
      results.push({ id: row.id, ok: true, updated_at: now.toISOString() });
    } catch (e) {
...
notesRoute.get('/pull', async (c) => {
  ...
  if (metaRows.length === 0) return c.json({ rows: [] });
  ...
  // PERFORMANCE NOTE: this is one Drive API read per changed note. Fine for
  // incremental pulls (usually a handful of notes); a first-install full
  // pull on an account with many notes will be slow and could hit Drive's
  // per-user rate limit. Worth batching/caching before that's a real user's
  // experience, not before then.
  ...
  return c.json({ rows, cursor: new Date().toISOString() });
});
...
notesRoute.delete('/', async (c) => {
  ...
  if (all.length > 0) {
    const { accessToken, refreshToken } = await getLiveGoogleTokens(db, userId);
    await Promise.all(all.map((n) => deleteNoteFile(accessToken, refreshToken, n.driveFileId).catch(() => {})));
  }
